...
Int8EntropyCalibrator::Int8EntropyCalibrator(int batchsize, std::vector<int> sizebuffers, int input_w, int input_h,
        const char* img_dir,
        const char* calib_cache_name_,
        const char* input_blob_name,
        bool read_cache):
        batchsize_(batchsize),
        sizebuffer_(sizebuffers),
        input_w_(input_w),
        input_h_(input_h),
        img_dir_(img_dir),
        calib_cache_name_(calib_cache_name_),
        input_blob_name_(input_blob_name),
        read_cache_(read_cache)
{
    CHECK(cudaMalloc(&device_input_, batchsize_ * sizebuffer_[0] * sizeof(float)));
...
bool Int8EntropyCalibrator::getBatch(void* bindings[], const char* names[], int nBinding) noexcept
{
    ...
    for (int i = img_idx_; i < img_idx_ + batchsize_; i++)
    {
        ...
        input_images.push_back(pr_img);
    }
    img_idx_ += batchsize_;
    cv::Mat blob = cv::dnn::blobFromImages(input_images, 1.0 / 255.0, cv::Size(input_w_, input_h_), cv::Scalar(0,0,0), true, false);

    CHECK(cudaMemcpy(device_input_, blob.ptr<float>(0), batchsize_ * sizebuffer_[0] * sizeof(float), cudaMemcpyHostToDevice));
